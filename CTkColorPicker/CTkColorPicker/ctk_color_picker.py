...
class AskColor(customtkinter.CTkToplevel):
    ...
    def __init__(self,
        ...
        # RGB entry addition start
        self.rgb_var = tkinter.StringVar()
        self.rgb_var.set("{},{},{}".format(*self.rgb_color))
        self.rgb_entry = customtkinter.CTkEntry(
            master=self.frame,
        ...
        self.rgb_entry.pack(pady=(8, 4))
        self.rgb_entry.bind("<Return>", self.on_rgb_entry)
        # RGB entry addition end
    ...
    def update_colors(self):
        ...
        # sync RGB entry to latest color
        self.rgb_var.set("{},{},{}".format(*self.rgb_color))
            
    def on_rgb_entry(self, event):
        try:
            rgb = [int(x.strip()) for x in self.rgb_var.get().split(",")]
            if not all(0 <= v <= 255 for v in rgb):
                raise ValueError
            self.rgb_color = rgb
            hexstr = "#{:02x}{:02x}{:02x}".format(*rgb)
            self.label.configure(fg_color=hexstr, text=hexstr)
            self.slider.configure(progress_color=hexstr)
        except Exception:
            self.rgb_var.set("{},{},{}".format(*self.rgb_color))
